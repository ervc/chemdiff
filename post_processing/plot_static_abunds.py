			...
			for spec in args.species:
				...
				if args.deuterium:
					...
					if dspec in x:
						...
						y[dspec] = [Y]
				if args.grains:
					gspec = get_ice_species(spec)
					if gspec in abundict:
						X = abundict[gspec][-1]
						if gspec in x:
							x[gspec].append(X)
							y[gspec].append(Y)
...
if __name__ == '__main__':
	...
	parser.add_argument('-d','--deuterium',help='include deuterated species',action='store_true')
	parser.add_argument('-g','--grains',help='include ice (grain) species',action='store_true')
